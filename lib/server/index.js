...
function cancelChanges(socket, messageId) {
  var socketChanges = allChanges['$' + socket.id];
  
  if (socketChanges && socketChanges[messageId]) {
    socketChanges[messageId].cancel();
    removeChangeHandler(socket, messageId);
  }
}
...
function onReceiveMessage(socket, type, messageId, args, pouchCreator) {
  ...
  switch (type) {
    ...
    case 'cancelChanges':
      return cancelChanges(socket, messageId);
    case 'destroy':
...
function cancelAllChanges(socket) {
  var socketChanges = allChanges['$' + socket.id];
  ...
  log('cancelAllChanges', socket.id);
  var messageId;
  for(messageId in socketChanges){
	  cancelChanges(socket, messageId);
  }
}

function storeChangeHandle(socket, messageId, handle){
  var socketChanges = allChanges['$' + socket.id];
  
  if(!socketChanges){
    socketChanges = {};
    allChanges['$' + socket.id] = socketChanges;
  }
  socketChanges[messageId] = handle;
}

function removeChangeHandler(socket, messageId){
  var socketChanges = allChanges['$' + socket.id];
  
  if(!socketChanges){
    return;
  }
  delete socketChanges[messageId];
  
  if(Object.keys(socketChanges).length === 0){
    delete allChanges['$' + socket.id];
  }
}
